...
awk -F ' ' '{print $1" "$5" "$6" "$3" "$4}' $1 > step1.jo #input file is output of the final report merger, considered the reference population for SNP coding.
...
echo "ID SNP_ID TOP_Genotype GCTA_Genotype" >final_header.jo
cat final_header.jo done_noheader.jo | tr ' ' '\t'> $3_equivalence.txt
awk -F ' ' '{print $1" "$2" "$4}' $3_equivalence.txt | sort -k1 > $3_GCTA_genotypes_long.txt
